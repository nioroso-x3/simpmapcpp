...
cv::Mat MapTileRenderer::drawMap(double latitude, double longitude, double zoom,
    ...
    if (layers) {
        ...
        // Effective meters/pixel of the oversized canvas at the requested
        // fractional zoom. The dpi_scale factor is folded in because the
        // canvas is already dpi_scale * diag pixels wide.
